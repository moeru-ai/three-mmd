...
class MMDLoader extends Loader {
  ...
  loadPMD(url, onLoad, onProgress, onError) {
    this.loader
      ...
      .load(url, (buffer) => {
        try {
          onLoad(MMDParser.parsePmd(buffer, true))
        }
        catch (e) {
          if (onError)
            onError(e)
  ...
  loadPMX(url, onLoad, onProgress, onError) {
    this.loader
      ...
      .load(url, (buffer) => {
        try {
          onLoad(MMDParser.parsePmx(buffer, true))
        }
        catch (e) {
          if (onError)
            onError(e)
  ...
  build(data) {
    ...
    if (data.metadata.format === 'pmx') {
      ...
      for (let i = 0; i < data.metadata.boneCount; i++) {
        ...
        grantEntryMap[i] = { children: [], param, parent: null, visited: false }
      }

      const rootEntry = { children: [], param: null, parent: null, visited: false }
